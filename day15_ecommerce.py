...
def login():
    ...
    if user_login == True:
        ...
        if type == "buyer":
            ...
            user_operation = input("[view products/ view bills] :").lower() 
...
def purchase(username):
    ...
    f = open('day15_productdata.txt','r')
    ...
    if available == True:
        ...
        bill_data = {'buyer':username,'product':product_dict_data.get('product_name'),'quantity':purhcase_quantity,'product_price':product_dict_data.get('product_price'),'total':a}

        bill_json_data = json.dumps(bill_data)

        f = open('day15_bill.txt','a')
...
def add_product():
    ...
    f = open('day15_productdata.txt','a')
